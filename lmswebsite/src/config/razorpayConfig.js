export const razorPayKeys = {
    // key_id: 'rzp_test_YW49ucyVtjePLT',//development
    key_id:'rzp_live_nmEtTvImeXIvf7', //production
    // key_secret: 'FFn6tCkoKKQxfdvPCFuiijRE', //development
    key_secret:'u4QbAJziwXhGYmdFWamz2KkZ',//production
    name: 'The Topper Academy',
    logo:'https://firebasestorage.googleapis.com/v0/b/demoproject-6d5cd.appspot.com/o/image%20(4).png?alt=media&token=29dfa005-15a7-4d7c-a0d9-410a149ebeee'
}