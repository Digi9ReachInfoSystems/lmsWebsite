...
import './UserEngagementChart.css'; // Custom CSS
...
const UserEngagementChart = () => {
  ...
  const years = [{ label: 'None', value: '' }, ...Array.from({ length: 23 }, (_, i) => ({
    label: 2000 + i,
    value: 2000 + i,
  }))];

  const months = [{ label: 'None', value: '' }, ...Array.from({ length: 12 }, (_, i) => ({
    label: new Date(0, i).toLocaleString('default', { month: 'long' }),
    value: i + 1,
  }))];

  const days = [{ label: 'None', value: '' }, ...Array.from({ length: 31 }, (_, i) => ({
    label: i + 1,
    value: i + 1,
  }))];
  ...
  const buildQueryParams = () => {
    ...
    if (filters.year&&filters.year!=='None') query.append('year', filters.year);
    if (filters.month && filters.month!=='None') query.append('month', filters.month);
  ...
  const fetchChartData = async () => {
    try {
      ...
      setChartData({
        labels,
        datasets: [
          { label: 'Paid Users', data: paidData, borderColor: '#7e7e7e', borderWidth: 2 },
          { label: 'Unpaid Users', data: unpaidData, borderColor: '#F72585', borderWidth: 2 },
        ],
      });
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="chart-wrapper">
      ...
      {loading ? (
        ...
        <Chart className="user-engagement-chart" type="line" data={chartData} options={chartOptions} />
      )}
    </div>
  );
};
