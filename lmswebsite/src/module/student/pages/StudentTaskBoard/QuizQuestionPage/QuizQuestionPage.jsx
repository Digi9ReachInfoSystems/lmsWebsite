...
const QuizQuestionPage = () => {
    ...
    const checkIfSubmitted = async () => {
      try {
        ...
        const studentResponse = responses.find(
          (res) => res.student_id._id === studentId
        );
        if (studentResponse) {
          message.warning(
            "You have already submitted responses for this quiz."
          );
  ...
  const handleSubmitAnswers = async (values) => {
    ...
    try {
      ...
      if (result && result.response) {
        message.success(
          `Your score is: ${result.response.score}/${quiz.questions.length}`
        );
      }
      ...
      if (
        ...
        error.response?.data?.error ===
          "Response already submitted for this quiz."
      ) {
  ...
  return (
    <div>
      ...
      <Subheading style={{ textAlign: "center", paddingBottom: "30px" }}>
        {quiz.quiz_title}
      </Subheading>
      <Form onFinish={handleSubmitAnswers}>
        ...
        <Form.Item>
          <PrimaryButton type="submit">Submit Answers</PrimaryButton>
        </Form.Item>
      </Form>
    </div>
  );
